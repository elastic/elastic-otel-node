...
const testFixtures = [
    {
        ...
        env: {
            ...
            ELASTIC_OTEL_INSTRUMENTATION_HTTP_CLIENT_CAPTURE_REQUEST_HEADERS: 'foo,spam,connection,traceparent,twice',
            ELASTIC_OTEL_INSTRUMENTATION_HTTP_CLIENT_CAPTURE_RESPONSE_HEADERS: 'server,content-type,deuxfois',
            ELASTIC_OTEL_INSTRUMENTATION_HTTP_SERVER_CAPTURE_REQUEST_HEADERS: 'foo,spam,connection,traceparent,twice',
            ELASTIC_OTEL_INSTRUMENTATION_HTTP_SERVER_CAPTURE_RESPONSE_HEADERS: 'server,content-type,deuxfois',
        },
        // verbose: true,
        checkTelemetry: (t, col) => {
            ...
            t.deepEqual(span.attributes['http.response.header.content_type'], [ 'text/plain' ]);

            span = spans[1];
            t.deepEqual(span.scope.name, '@opentelemetry/instrumentation-http');
            t.strictEqual(span.kind, 'SPAN_KIND_SERVER');
            t.deepEqual(span.attributes['http.request.header.foo'], [ 'Bar' ]);
            t.deepEqual(span.attributes['http.request.header.connection'], [ 'keep-alive' ]);
            t.deepEqual(span.attributes['http.request.header.traceparent'], [ `00-${span.traceId}-${span.parentSpanId}-01` ]);
            t.deepEqual(span.attributes['http.response.header.server'], [ 'capture-header-example' ]);
            t.deepEqual(span.attributes['http.response.header.content_type'], [ 'text/plain' ]);

            span = spans[2];
            t.deepEqual(span.scope.name, '@opentelemetry/instrumentation-undici');
            t.strictEqual(span.kind, 'SPAN_KIND_CLIENT');
