...
function getOtlpExporter() {
  ...
  switch(otlpProtocol) {
    ...
    case 'http/protobuf':
      ...
      break;
    default:
      log.warn(`Unsupported OTLP traces protocol: ${otlpProtocol}. Using http/protobuf.`);
      exporterPkgName += 'proto';
  }
...
function getSpanProcessors(processors) {
  ...
  console.log('getSpanProcessors', metricsEnabled, Array.isArray(processors))
  ...
  if (exporters.some(exp => exp === 'none')) {
    log.warn('OTEL_TRACES_EXPORTER contains "none". No trace information or Spans will be exported.');
    return [];
  }

  if (exporters.length === 0) {
    log.debug('OTEL_TRACES_EXPORTER is empty. Using the default "otlp" exporter.')
    exporters.push('otlp');
  }
  ...
  for (const name of exporters) {
    switch(name) {
      case 'otlp':
        result.push(new BatchSpanProcessor(getOtlpExporter()));
        break;
      case 'console':
        result.push(new SimpleSpanProcessor(new ConsoleSpanExporter()));
        break;
      case 'zipkin':
        const {ZipkinExporter} = require('@opentelemetry/exporter-zipkin');
        result.push(new BatchSpanProcessor(new ZipkinExporter()));
        break;
      case 'jaeger':
        ...
        log.warn(`OTEL_TRACES_EXPORTER value "${name}" not available yet.`);
        break;
      default:
  ...
  console.log('getSpanProcessors.length', result)
  return result;
}
